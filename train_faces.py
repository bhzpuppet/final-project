...
train_x,test_x,train_y,test_y = train_test_split(imgs, labs, test_size=0.1, random_state=1)
...
def cnnLayer():
    ...
    # 全连接层1
    # Wf1 = weightVariable([2 * 2 * 512, 512])
    Wf1 = tf.Variable(initial_value=tf.random_normal([2 * 2 * 512, 512], stddev=0.01), name='fc1')
...
def cnnTrain():
    ...
    train_step = tf.train.AdamOptimizer(0.1).minimize(final_loss, global_step=global_step)
        ...
        for n in range(30):
            ...
            for i in range(num_batch):
                ...
                _,loss,summary = sess.run([train_step, cross_entropy, merged_summary_op],
                                           feed_dict={x:batch_x,y_:batch_y, keep_prob_5:1.0,keep_prob_75:0.7})
                ...
                if (n*num_batch+i) % 100 == 0:
                    ...
                    # 准确率大于0.98时保存并退出
                    if acc > 0.985 and n > 2:
                        saver.save(sess, './model/relu_fc1.model', global_step=n * num_batch + i)
